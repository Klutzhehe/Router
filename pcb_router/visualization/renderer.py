    ...
    def render_board(
        ...
        for seg in board_state.traces:
            if show_all_layers or seg.layer == layer:
                ...
                alpha = 1.0 if (not show_all_layers or seg.layer == layer) else 0.25
                ax.plot(
        ...
        for via in board_state.vias:
            if show_all_layers or (min(via.from_layer, via.to_layer) <= layer <= max(via.from_layer, via.to_layer)):
                color = self._get_net_color(via.net_id)
                # Concentric circles
                ...
                outer_circle = patches.Circle(
                    (via.x, via.y), r_outer,
                    facecolor=color, edgecolor='#FFFFFF', linewidth=0.5, alpha=0.9
                )
                inner_circle = patches.Circle(
                    (via.x, via.y), r_inner,
                    facecolor=self.bg_color, edgecolor=color, linewidth=0.5, alpha=1.0
                )
